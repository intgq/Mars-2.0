...
static double error_limit = 3 * 1e-4;
static FILE* fp;
...
void updateCurrentTime(int thread) {
    ...
    for (int i = 0; i < numThread; i++) {
        if (threadState[i] != STATE_STOPPED && localTime[i] < minLocal) {
...
void init(int threadNumber, int channelNumber) {
    fp = fopen("./hcsp2c/system_test.log", "w");
    if (fp == NULL) {
        printf("fileError");
        assert(0);
    }
...
void input(int thread, Channel ch) {
    ...
    fprintf(fp, "Input of thread %d start.\n", thread);
    fflush(fp);
    ...
    pthread_mutex_unlock(&mutex);
    fprintf(fp, "Input of thread %d complete.\n", thread);
    fflush(fp);
}
...
void output(int thread, Channel ch) {
    ...
    fprintf(fp,"Output of thread %d start.\n", thread);
    fflush(fp);
    ...
    pthread_mutex_unlock(&mutex);
    fprintf(fp,"Output of thread %d complete.\n", thread);
    fflush(fp);
}
...
int externalChoice(int thread, int nums, Channel* chs) {
    // Take the global lock
    pthread_mutex_lock(&mutex);
    fprintf(fp,"External Choice of thread %d start.\n", thread);
    fflush(fp);
    ...
    for (int i = 0; i < nums; i++) {
        ...
        if (chs[i].type == 0) {
            ...
            if (channelOutput[curChannel] != -1 &&
                ...
                pthread_mutex_unlock(&mutex);  
                fprintf(fp,"External Choice of thread %d complete.\n", thread);
                fflush(fp);
                return i;
            }
        } else {
...
void interruptInit(int thread, int nums, Channel* chs) {
    // Take the global lock
    pthread_mutex_lock(&mutex);
    fprintf(fp,"Interrupt of thread %d start.\n", thread);
    fflush(fp);
    int curChannel;
...
int interruptPoll(int thread, double seconds, int nums, Channel* chs) {
    ...
    for (int i = 0; i < nums; i++) {
        ...
        if (chs[i].type == 0) {
            ...
            if (channelOutput[curChannel] != -1 &&
                ...
                pthread_mutex_unlock(&mutex);  
                fprintf(fp,"Interrupt of thread %d complete in channel %d with match index = %d.\n", thread, chs[i].channelNo, i);
                fflush(fp);
                return i;
            }
        } else {
    ...
    if (threadState[thread] == STATE_AVAILABLE) {
        // pthread_mutex_unlock(&mutex);
        fprintf(fp,"Interrupt of thread %d complete with match index = -1.\n", thread);
        fflush(fp);
        return -1;
    }
    ...
    pthread_mutex_unlock(&mutex);  
    fprintf(fp,"Interrupt of thread %d complete in channel %d with match index = %d.\n", thread, chs[match_index].channelNo, match_index);
    fflush(fp);
    return match_index;
}
...
void interruptClear(int thread, int nums, Channel* chs) {
    ...
    pthread_mutex_unlock(&mutex);
    fprintf(fp,"Interrupt of thread %d complete.\n", thread);
    fflush(fp);
    return;
}
