...
void* tran_veh_v (void* arg) {
    ...
    data = 0.0;
    while (1) {
        channels[0].channelNo = outputs_lb_velo_voter_rb__lb_veh_v_rb_;
        channels[0].type = 0;
        channels[0].pos = &data;
        channels[1].channelNo = inputs_lb_emerg_rb__lb_veh_v_rb_;
        channels[1].type = 1;
        channels[1].pos = &data;
        channels[2].channelNo = inputs_lb_PI_ctr_rb__lb_veh_v_rb_;
        channels[2].type = 1;
        channels[2].pos = &data;
        interruptInit(threadNumber, 3, channels);
        h = step_size;
        is_comm = 0;
        while (1) {
            double midTime = localTime[threadNumber];
            midInt = interruptPoll(threadNumber, h, 3, channels);
            if (midInt >= 0) {
            ...
            if (!(true)) {
                interruptClear(threadNumber, 3, channels);
                midInt = -1;
                break;
            }
