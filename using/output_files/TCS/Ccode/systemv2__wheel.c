...
void* wheel (void* arg) {
    threadNumber = (int)(*((int*)arg));
        while (1) {
        channel.channelNo = inputs_lb_PHY_vehicle_imp_rb__lb_veh_v_w_rb_;
        channel.type = 0;
        channel.pos = &data;
        ...
        *midDouble = 1.0;
        channels[0].channelNo = outputs_lb_wheel_rb__lb_wheel_valid_rb_;
        channels[0].type = 1;
        ...
        if (midInt == 0) {
            ...
            *midDouble = data + error;
            channel.channelNo = outputs_lb_wheel_rb__lb_wheel_v_rb_;
            channel.type = 1;
